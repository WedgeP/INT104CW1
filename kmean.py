...
df['Q4'] = df['Q4'] / 10
df['Q5'] = df['Q5'] / 6
df['Total'] = df['Total'] / 46  # 归一化
# 防止除零错误
epsilon = 1e-6

# 添加 1/x 形式的特征
df['1/Q1'] = 1 / (df['Q1'] + epsilon)
df['1/Q2'] = 1 / (df['Q2'] + epsilon)
df['1/Q3'] = 1 / (df['Q3'] + epsilon)
df['1/Q4'] = 1 / (df['Q4'] + epsilon)
df['1/Q5'] = 1 / (df['Q5'] + epsilon)
df['1/Total'] = 1 / (df['Total'] + epsilon)
...
# 使用PCA进行降维并查看数据分布
plt.figure(figsize=(10, 8))
scatter = plt.scatter(X_pca[:, 0], X_pca[:, 1], c=y.astype('category').cat.codes, cmap='viridis', alpha=0.7)
plt.title('PCA of Student Data')
plt.xlabel('Principal Component 1')
plt.ylabel('Principal Component 2')
plt.colorbar(scatter, label='Programme')  # 颜色条，显示不同的Programme类别
plt.grid(True, linestyle='--', alpha=0.7)
plt.show()
